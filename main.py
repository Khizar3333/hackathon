...
client = OpenAI(
    api_key=os.getenv("OPEN_API_KEY"),  # Replace with your actual API key
...
# Reset session state if it's the first run
...
st.sidebar.title("NutriCo Assistant")
st.sidebar.header("Welcome to NutriCo!")
st.sidebar.write("""
    NutriCo is your friendly nutrition assistant, here to help you with all your dietary needs.
    Please provide your details to get personalized nutrition advice.
    
    ### Features:
    - Ask questions about nutrition, diets, and healthy eating.
    - Get personalized meal suggestions based on your preferences.
    - Learn about vitamins, minerals, and their benefits.
    
    Feel free to interact with the chatbot below!
""")

# Function to filter nutrition-related responses
def is_nutrition_related(response):
    nutrition_keywords = ["nutrition", "diet", "healthy", "food", "calories", "vitamins", "minerals", "meal", "recipe"]
    return any(keyword in response.lower() for keyword in nutrition_keywords)
    ...
    if not user_details_collected():
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="o1-mini",
                messages=[
                    {"role": "assistant", "content": f"You are NutriCo, a friendly and helpful nutrition assistant. The user details are: Name - {st.session_state.user_details['name']}, Age - {st.session_state.user_details['age']}, Gender - {st.session_state.user_details['gender']}, Dietary Preferences - {st.session_state.user_details['dietary_preferences']}.give answer to the user question only those are related to nutrition and diet and tell the user in a friendly and quiet way if he ask unrelated question."},
                    *[
